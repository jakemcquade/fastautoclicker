...
export function HotkeyPopup({ hotkey, open, onClose, onSave }: HotkeyPopupProps) {
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (event: KeyboardEvent) => {
            ...
            // First key: if it's a modifier, start building a combo; otherwise record as bare key
            if (recordedInput.length === 0) {
                if (MODIFIERS.includes(key)) {
                    setRecordedInput([key]);
                } else {
                    setRecordedInput([key]);
                    setWatching(false);
                }
            ...
            // Second key: must not be a modifier (completes the combo)
            if (MODIFIERS.includes(key)) return;
            if (recordedInput.includes(key)) return;
            setRecordedInput([...recordedInput, key]);
            setWatching(false);
        };
    ...
    useEffect(() => {
        if (!open) {
            void commands.setState("hotkey_recording", false);
            return;
        }

        void commands.setState("hotkey_recording", watchingInput);
    }, [open, watchingInput]);
                ...
                <div className="bg-[rgba(var(--tertiary),1)] rounded-[8px] p-6 text-white shadow-lg border border-black/40">
                    <div className="flex flex-col gap-1 w-full">
                        ...
                        <p className="text-xs text-white/70 mb-2">
                            Press <span className="font-semibold">Start</span> and then your desired key.<br />
                            Any single key works (e.g. <kbd>F6</kbd>), or a modifier combo (e.g. <kbd>Ctrl</kbd>+<kbd>K</kbd>).<br />
                            Press <kbd>Escape</kbd> to cancel recording.
                        </p>
